...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def compute_cos_sin_cache(self):
        ...
        base_thetas = 1.0 / (
            ...
            ** (
                ...
                torch.arange(0, self.d_k - 1, 2, device=self.device).float()
                / self.d_k
            )
        )
    ...
    def forward(
        ...
        return rearrange(
            x_rotated, "... seq_len d_k_half rot_row -> ... seq_len (d_k_half rot_row)"
        )
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        assert (rope_max_seq_len is None) == (
            rope_theta is None
        ), "both rope_max_seq_len and rope_theta should be set or both should be None"
        self.rope = None
